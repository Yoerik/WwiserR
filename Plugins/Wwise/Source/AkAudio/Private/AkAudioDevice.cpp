...
// WWISSERR BEGIN
AKRESULT FAkAudioDevice::SetSwitch(AkSwitchGroupID in_SwitchGroup, AkSwitchStateID in_SwitchState, UAkComponent* in_pComponent)
{
	AkGameObjectID GameObjID = in_pComponent->GetAkGameObjectID();

	if (m_bSoundEngineInitialized && GameObjID != AK_INVALID_GAME_OBJECT)
	{
		auto* SoundEngine = IWwiseSoundEngineAPI::Get();
		if (UNLIKELY(!SoundEngine)) return AK_NotInitialized;

		AKRESULT eResult = SoundEngine->SetSwitch(in_SwitchGroup, in_SwitchState, GameObjID);
		return eResult;
	}
...
void FAkAudioDevice::AddDefaultListener(UAkComponent* in_pListener, UWorld* world, bool bSilent)
{
	if (!IsValid(world)) { return; }
//	if (UWorld* currentWorld = GEngine->GetWorld())
	{
		//if (world != currentWorld) { return; }
		//AddDefaultListener(in_pListener);
