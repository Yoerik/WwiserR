...
void UAkComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	if (SoundEngine->IsInitialized())
	{
		...
		if (!HasActiveEvents())
		{
			if (!bAutoDestroy)
			{
				if (OnAllEventsEnded.IsBound())
				{
					// on next tick to make sure we still receive (latent) callbacks before destroying the AkComp
					//World->GetTimerManager().SetTimerForNextTick([this]() { OnAllEventsEnded.Execute(this); });		
					OnAllEventsEnded.Execute(this);
				}
			}
			else if (bEventPosted)
			{
				DestroyComponent();
			}
		}
